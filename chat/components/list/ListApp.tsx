...
import { supabase, getFastSession } from "@/lib/list/SupabaseClient";
...
import { useBulkMarkAsPublic } from "@/hooks/list/useBulkMarkAsPublic";
import {
  useMarkdownExtraction,
  MarkdownProgressItem,
} from "@/hooks/list/useMarkdownExtraction";
import {
  useYouTubePlaylistExtraction,
  YouTubeProgressItem,
} from "@/hooks/list/useYouTubePlaylistExtraction";
import { useLibgenSearch } from "@/hooks/list/useLibgenSearch";
import { useAudioTranscription } from "@/hooks/list/useAudioTranscription";
import { useYouTubeTranscript } from "@/hooks/list/useYouTubeTranscript";
import { useTagsForGroup } from "@/hooks/list/useTagQueries";
import { useToast } from "./ToastProvider";
import { extractUrls, getFirstUrl } from "@/utils/list/urlDetection";
import { getYouTubeVideoFromContent } from "@/utils/list/youtubeHelpers";
...
export const ListApp: React.FC = () => {
  ...
  const [groupModalMode, setGroupModalMode] = useState<"create" | "join">(
    "create",
  );
  const [activeExternalSearch, setActiveExternalSearch] = useState<
    string | null
  >(null);
  ...
  const [markdownProgressItems, setMarkdownProgressItems] = useState<
    MarkdownProgressItem[]
  >([]);
  const [selectedContentForMarkdown, setSelectedContentForMarkdown] = useState<
    Content[]
  >([]);

  // YouTube Playlist Progress tracking
  const [showYouTubeProgress, setShowYouTubeProgress] = useState(false);
  const [youTubeProgressItems, setYouTubeProgressItems] = useState<
    YouTubeProgressItem[]
  >([]);
  const [selectedContentForYouTube, setSelectedContentForYouTube] = useState<
    Content[]
  >([]);

  // TMDb Search modal state
  const [showTMDbModal, setShowTMDbModal] = useState(false);
  const [selectedContentForTMDb, setSelectedContentForTMDb] =
    useState<Content | null>(null);
  const [tmdbSearchQuery, setTmdbSearchQuery] = useState<string | undefined>(
    undefined,
  );

  // Libgen Search modal state
  const [showLibgenModal, setShowLibgenModal] = useState(false);
  const [selectedContentForLibgen, setSelectedContentForLibgen] = useState<
    Content[]
  >([]);
  ...
  const [currentContentForSharing, setCurrentContentForSharing] =
    useState<Content | null>(null);
  ...
  const [selectedContentForClaudeCode, setSelectedContentForClaudeCode] =
    useState<Content[]>([]);

  // YouTube Video Annotator modal state
  const [showYouTubeAnnotator, setShowYouTubeAnnotator] = useState(false);
  const [selectedVideoForAnnotation, setSelectedVideoForAnnotation] =
    useState<Content | null>(null);
  ...
  const handleSEOExtraction = async () => {
    ...
    try {
      ...
      const validContent = selectedContent.filter(
      ...
      if (validContent.length === 0) {
        toast.error(
          "No content found",
          "Unable to find selected items in database.",
        );
        return;
      }
      ...
      const initialProgressItems: SEOProgressItem[] = validContent.map(
        (content) => ({
      ...
      console.log(
        "Starting SEO extraction for:",
        validContent.map((c) => c.id),
      );
  ...
  const handleStartMoveToParent = () => {
    ...
    toast.info(
      "Select new parent",
      'Click any item to set as parent, or navigate and click "Move Here"',
    );
  };
  ...
  const handleConfirmMove = async () => {
    ...
    try {
      ...
      if (targetParentId) {
        try {
          const parentContent =
            await contentRepository.getContentById(targetParentId);
          parentName =
            parentContent.data.substring(0, 30) +
            (parentContent.data.length > 30 ? "..." : "");
        } catch (error) {
  ...
  const handleMarkdownExtraction = async () => {
    ...
    try {
      ...
      // Initialize progress tracking
      const initialProgressItems: MarkdownProgressItem[] = validContent.map(
        (content) => ({
          contentId: content.id,
          content,
          status: "pending" as const,
      ...
      console.log(
        "Starting markdown extraction for:",
        validContent.map((c) => c.id),
      );
  ...
  const handleYouTubePlaylistExtraction = async () => {
    ...
    try {
      ...
      // Initialize progress tracking
      const initialProgressItems: YouTubeProgressItem[] = validContent.map(
        (content) => ({
          contentId: content.id,
          content,
      ...
      console.log(
        "Starting YouTube playlist extraction for:",
        validContent.map((c) => c.id),
      );
      ...
      // Show results summary
      const totalPlaylists = results.reduce(
        (sum, r) => sum + r.playlists_found,
        0,
      );
  ...
  const handleTMDbSearch = async () => {
    ...
    if (selectedIds.length > 1) {
      toast.warning(
        ...
        "Please select only one content item to search TMDb",
      );
      return;
    }
    ...
    try {
      ...
      if (!content) {
        toast.error(
          "Content not found",
          "Unable to find selected item in database",
        );
        return;
      }
  ...
  const handleLibgenSearch = async () => {
    ...
    try {
      ...
      console.log(
        "Opening Libgen search modal for content:",
        validContent.map((c) => c.id),
      );
    } catch (error) {
  ...
  const handleAddSelectedLibgenBooks = async (books: Content[]) => {
    ...
    try {
      ...
      console.log(
        `Successfully created ${createdBooks.length} book content items`,
      );
  ...
  const handleLibgenSearchExecute = async (config: LibgenSearchConfig) => {
    ...
    try {
      ...
      const totalBooksFound = results.reduce(
        (sum, r) => sum + r.books_found,
        0,
      );
  ...
  const handleClaudeCodeExecution = async () => {
    ...
    try {
      ...
      const defaultPrompt =
        "Create an interactive TSX component based on the selected content. Make it visually appealing and functional.";

      toast.info(
        "Claude Code Starting",
        "Generating component from selected content...",
      );
      ...
      if (!response.session_id) {
        throw new Error(
          "Invalid response from Claude Code Lambda - missing session_id",
        );
      }
    ...
    try {
      ...
      validContent.forEach((content) => {
        ...
        if (content.metadata?.url_preview) {
          ...
          console.log(
            `Content ${content.id} already has screenshot: ${content.metadata.url_preview}`,
          );
          return;
        }
      ...
      console.log(
        "Content with existing screenshots:",
        contentWithExistingScreenshots,
      );
      ...
      if (urlsToProcess.length === 0) {
        if (contentWithoutUrls > 0 && contentWithExistingScreenshots === 0) {
          toast.info(
            "No URLs Found",
            "Selected content items do not contain any URLs to screenshot.",
          );
        } else if (contentWithExistingScreenshots > 0) {
          toast.info(
            "Already Complete",
            "All selected content items already have screenshots.",
          );
        } else {
          toast.info(
            "Nothing to Process",
            "No content items available for screenshot generation.",
          );
        }
        return;
      }
      ...
      toast.info(
        ...
        `Starting screenshot generation for ${urlsToProcess.length} URL${urlsToProcess.length > 1 ? "s" : ""}...`,
      );
      ...
      for (const { url, contentId } of urlsToProcess) {
        try {
          console.log(
            `Generating screenshot for ${url} (content: ${contentId})`,
          );

          const result = await contentRepository.generateUrlPreview(
            contentId,
            url,
          );
          ...
          if (result.job_id) {
            ...
            console.log(
              `Screenshot job queued for ${contentId}: ${result.job_id}`,
            );
          } else if (result.success && result.screenshot_url) {
            // Immediate success (shouldn't happen with queue)
            await contentRepository.updateContentUrlPreview(
              contentId,
              result.screenshot_url,
            );
            console.log(
              `Successfully generated screenshot for ${contentId}: ${result.screenshot_url}`,
            );
            successCount++;
          } else {
            // Actual failure
            console.error(
              `Failed to generate screenshot for ${contentId}:`,
              result.error,
            );
            failureCount++;
          }
      ...
      if (queuedJobs.length > 0 && successCount === 0 && failureCount === 0) {
        toast.info(
          "Jobs Queued",
          `${queuedJobs.length} screenshot job${queuedJobs.length > 1 ? "s" : ""} queued for processing`,
        );
      } else if (successCount > 0 && failureCount === 0) {
        toast.success(
          "Screenshots Generated",
          `Successfully generated ${successCount} screenshot${successCount > 1 ? "s" : ""}!`,
        );
        ...
        toast.error(
          "Generation Failed",
          "Failed to generate any screenshots. Please try again later.",
        );
      }
      ...
      toast.error(
        "Error",
        "Failed to process selected content for screenshot generation",
      );
    }
  };
  ...
  const handleTranscribeAudio = async () => {
    ...
    try {
      ...
      if (audioContent.length === 0) {
        toast.info(
          "No Audio Files",
          "Selected content items do not contain any audio files to transcribe.",
        );
        return;
      }
      ...
      toast.info(
        "Transcribing Audio",
        `Starting transcription for ${audioContent.length} audio file${audioContent.length > 1 ? "s" : ""}...`,
      );
      ...
      if (successCount > 0 && failureCount === 0) {
        ...
        toast.info(
          "Jobs Queued",
          `${audioContent.length} transcription job${audioContent.length > 1 ? "s" : ""} queued for processing`,
        );
      } else {
        toast.error(
          "Transcription Failed",
          "Failed to transcribe any audio files. Please try again later.",
        );
      }
      ...
      toast.error(
        "Error",
        "Failed to process selected content for transcription",
      );
    }
  };
  ...
  const handleYouTubeTranscript = async () => {
    ...
    try {
      ...
      if (youtubeContent.length === 0) {
        toast.info(
          "No YouTube Videos",
          "Selected content items do not contain any YouTube videos to extract transcripts from.",
        );
        return;
      }
      ...
      toast.info(
        ...
        `Starting transcript extraction for ${youtubeContent.length} YouTube video${youtubeContent.length > 1 ? "s" : ""}...`,
      );
      ...
      if (successCount > 0 && failureCount === 0) {
        ...
        toast.info(
          "Jobs Queued",
          `${youtubeContent.length} transcript extraction job${youtubeContent.length > 1 ? "s" : ""} queued for processing`,
        );
      } else {
        toast.error(
          "Transcript Extraction Failed",
          "Failed to extract any transcripts. Please try again later.",
        );
      }
      ...
      toast.error(
        "Error",
        "Failed to process selected content for transcript extraction",
      );
    }
  };
  ...
  const handleYouTubeAnnotation = async () => {
    ...
    if (selectedIds.length > 1) {
      toast.warning(
        "Multiple items selected",
        "Please select only one YouTube video to annotate",
      );
      return;
    }
    ...
    try {
      ...
      if (!videoId) {
        toast.warning(
          "Not a YouTube video",
          "Please select content containing a YouTube video URL",
        );
        return;
      }
  ...
  const handleSpotifyPlaylistImported = () => {
    ...
    toast.success(
      "Playlist Imported!",
      "Successfully imported Spotify playlist",
    );
  };
  ...
  const handleAIContentAnalysis = async () => {
    ...
    try {
      ...
      const serializedContent = selectedContent
        .map((item, index) => {
  ...
  const handleBulkMarkAsPublic = async () => {
    ...
    if (selectedIds.length === 0) {
      toast.error(
        "No content selected",
        "Please select content to mark as public",
      );
      return;
    }
  ...
  // Define available workflow actions - memoized to prevent unnecessary re-renders
  // Note: onClick handlers use refs to access latest state without recreating the array
  const workflowActionsRef = useRef<{
    handleQuickAIChat: () => void;
    handleAIContentAnalysis: () => void;
    handleClaudeCodeExecution: () => void;
    handleSEOExtraction: () => void;
    handleMarkdownExtraction: () => void;
    handleYouTubePlaylistExtraction: () => void;
    handleLibgenSearch: () => void;
    handleYouTubeAnnotation: () => void;
    handleTMDbSearch: () => void;
    handleScreenshotGeneration: () => void;
    handleTranscribeAudio: () => void;
    handleYouTubeTranscript: () => void;
    setShowSendToGroupModal: (v: boolean) => void;
    handleStartMoveToParent: () => void;
    handleBulkMarkAsPublic: () => void;
    ...
    setShowSendToGroupModal: () => {},
    handleStartMoveToParent: () => {},
  ...
  const workflowActions = useMemo(
    () => [
      ...
      },
      {
        ...
        onClick: () => {
          console.log(
            "💻 WorkflowAction onClick triggered for Claude Code execution",
          );
          workflowActionsRef.current.handleClaudeCodeExecution();
        },
      },
      {
        id: "seo-extract",
        ...
        onClick: () => {
          ...
          workflowActionsRef.current.handleSEOExtraction();
        },
      },
      {
        id: "markdown-extract",
        ...
        onClick: () => {
          console.log(
            "📝 WorkflowAction onClick triggered for markdown extraction",
          );
          workflowActionsRef.current.handleMarkdownExtraction();
        },
      },
      {
        id: "youtube-playlist",
        ...
        onClick: () => {
          console.log(
            "🎬 WorkflowAction onClick triggered for YouTube playlist extraction",
          );
          workflowActionsRef.current.handleYouTubePlaylistExtraction();
        },
      },
      {
        id: "libgen-search",
      ...
      },
      {
        ...
        onClick: () => {
          console.log(
            "📸 WorkflowAction onClick triggered for screenshot generation",
          );
          workflowActionsRef.current.handleScreenshotGeneration();
        },
      },
      {
        id: "transcribe-audio",
        ...
        onClick: () => {
          console.log(
            "🎙️ WorkflowAction onClick triggered for audio transcription",
          );
          workflowActionsRef.current.handleTranscribeAudio();
        },
      },
      {
        ...
        onClick: () => {
          console.log(
            "📜 WorkflowAction onClick triggered for YouTube transcript extraction",
          );
          workflowActionsRef.current.handleYouTubeTranscript();
        },
      },
      {
        id: "send-to-group",
        ...
        onClick: () => {
          ...
          workflowActionsRef.current.setShowSendToGroupModal(true);
        },
      },
      {
        id: "move-to-parent",
        ...
        icon: "📁",
        onClick: () => workflowActionsRef.current.handleStartMoveToParent(),
      },
      {
        id: "mark-as-public",
        name: "Mark as Public",
        ...
        onClick: () => {
          ...
          workflowActionsRef.current.handleBulkMarkAsPublic();
        },
      },
      {
        id: "bulk-delete",
        ...
        icon: "🗑️",
        onClick: () => workflowActionsRef.current.handleBulkDelete(),
      },
    ],
    [],
  );

  // Filter workflows by category for search workflows - memoized since workflowActions is now stable
  const searchWorkflows = useMemo(
    () =>
      workflowActions.filter((action) => action.category?.includes("search")),
    [workflowActions],
  );
      ...
      const {
        ...
        try {
          if (session?.user) {
            ...
            const isActualSignupCompletion =
              event === "SIGNED_IN" &&
            ...
            if (isActualSignupCompletion) {
              console.log(
                "Genuine signup completion detected - email confirmed",
              );
              ...
              if (pendingInviteCode || isOnInviteUrl) {
                ...
                toast.success(
                  "Email confirmed!",
                  "Welcome! Joining your group now...",
                );
              } else {
  ...
  const handlePendingInvite = async () => {
    ...
    try {
      ...
      const wasSignupCompletion =
        hasShownWelcomeRef.current ||
        localStorage.getItem("pendingSignupConfirmation") === "true";
      if (wasSignupCompletion) {
        toast.success(
          "Welcome! You've joined the group",
          `Successfully joined "${group.name}" after confirming your email.`,
        );
      } else {
  ...
  useEffect(() => {
    if (currentGroup?.id) {
      const savedViewMode = localStorage.getItem(
        `viewMode_${currentGroup.id}`,
      ) as ViewMode;
      if (
        savedViewMode &&
        ["chronological", "random", "alphabetical", "oldest"].includes(
          savedViewMode,
        )
      ) {
  ...
  useEffect(() => {
    if (currentGroup?.id) {
      const savedTagFilter = localStorage.getItem(
        `tagFilter_${currentGroup.id}`,
      );
      if (savedTagFilter) {
        try {
    ...
    if (currentGroup?.id) {
      if (selectedTagFilter.length > 0) {
        localStorage.setItem(
          `tagFilter_${currentGroup.id}`,
          JSON.stringify(selectedTagFilter),
        );
      } else {
  ...
  useEffect(() => {
    if (currentGroup?.id) {
      const savedDisplayMode = localStorage.getItem(
        `displayMode_${currentGroup.id}`,
      ) as DisplayMode;
      if (
        savedDisplayMode &&
        ["list", "stack", "masonry", "deck"].includes(savedDisplayMode)
      ) {
    ...
    const handleInviteNavigation = async () => {
      ...
      if (inviteMatch) {
        ...
        try {
          ...
          const group =
            await joinGroupMutationRef.current.mutateAsync(joinCode);
          ...
          setError(
            "Failed to join group. The invite code may be invalid or expired.",
          );
  ...
  return (
    <div
      ...
      className="bg-gray-50 flex flex-col transition-all duration-200 ease-in-out overflow-hidden"
    >
      {/* Error State */}
      {appState === "error" && (
      ...
      {/* Auth Screen */}
      {appState === "ready" && !user && <UserAuth onAuthSuccess={() => {}} />}
      ...
      {(appState === "loading" ||
        ...
        (appState === "ready" &&
          user &&
          !currentGroup &&
          (groupsLoading || groups.length > 0))) && <AppSkeleton />}
            ...
            <>
              {showYouTubeAnnotator ? (
                ...
                <>
                                    ...
                                    {selectedTagFilter.map((tagFilter) => (
                                      ...
                                      >
                                        ...
                                        <button
                                          ...
                                          style={{
                                            backgroundColor: tagFilter.tag.color
                                              ? `${tagFilter.tag.color}20`
                                              : "#e5e7eb",
                                            borderColor:
                                              tagFilter.tag.color || "#d1d5db",
                                            borderWidth: "1px",
                                            color:
                                              tagFilter.tag.color || "#374151",
                                          }}
                                      ...
                                      <button
                                        ...
                                        style={{
                                          backgroundColor: selectedTagFilter[
                                            ...
                                            ? `${selectedTagFilter[selectedTagFilter.length - 1].tag.color}20`
                                            : "#e5e7eb",
                                          borderColor:
                                            selectedTagFilter[
                                              selectedTagFilter.length - 1
                  ...
                  <div className="m-20 flex-1 flex flex-col max-w-4xl mx-auto w-full shadow-sm transition-all duration-200 ease-in-out">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-orange-700">
                                  Click any item to set as parent for{" "}
                                  {itemsToMove.length} selected item
                  ...
                  {isSelectingParent && (
                    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-lg">
                      <div className="max-w-md mx-auto p-4 flex gap-3">
                        ...
                        >
                          Move Here ({itemsToMove.length} item
                          {itemsToMove.length > 1 ? "s" : ""})
                        </button>
                        <button
                          onClick={handleCancelMoveToParent}
