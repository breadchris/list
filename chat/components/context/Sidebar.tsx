...
export const Sidebar = forwardRef<IPane, SidebarProps>(({ 
  ...
  return (
    <div className={cn(
        "h-full flex flex-col bg-muted overflow-hidden font-sans transition-all duration-500 ease-in-out",
        isExpanded ? "items-center pt-20 px-4" : "border-r border-border w-full"
    )}>
      
      {/* Input Section */}
      <div className={cn(
           "shrink-0 transition-all duration-500 ease-in-out w-full z-20",
           isExpanded ? "max-w-2xl mb-8" : "p-3 border-b border-border bg-background flex flex-col gap-2"
       )}>
           {isExpanded ? (
               <h1 className="text-3xl font-light text-foreground text-center mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                 {entries.length === 0 ? "What's on your mind?" : "just write"}
               </h1>
           ) : onBackToHome && (
               <button 
                   onClick={onBackToHome}
                   className="self-start flex items-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-indigo-600 transition-colors px-1 py-0.5"
               >
           ...
           <div className="relative group">
                ...
                <textarea
                    ...
                    className={cn(
                        "w-full bg-background text-foreground placeholder:text-muted-foreground outline-none resize-none transition-all",
                        isExpanded 
                            ...
                            : "rounded-xl p-3 text-sm shadow-sm border border-border focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 min-h-[80px] pr-2"
                    )}
                />
                <button 
      ...
      <div className={cn(
          ...
          isExpanded ? "max-w-2xl" : ""
            ...
            <motion.div 
            layout
            className="h-full overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-border w-full"
            >
                {sortedDates.length === 0 ? (
                    ...
                    sortedDates.map((dateKey) => (
                        <div key={dateKey} className="relative">
                            <div className="sticky top-0 z-10 bg-muted/95 backdrop-blur-sm pb-2 pt-2 border-b border-transparent">
                                <h3 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground ml-1">
                                ...
                                {groupedEntries[dateKey].map((entry) => {
                                    ...
                                    >
                                        <div 
                                            ...
                                            className={cn(
                                                "bg-background p-3 rounded-lg shadow-sm border border-border hover:shadow-md transition-all cursor-pointer hover:border-indigo-200 hover:translate-x-0.5",
                                                isExpanded ? "p-5" : "p-3",
                                            ...
                                            {editingEntryId === entry.id ? (
                                                <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
                                                    <textarea 
                                                        ...
                                                        className="w-full bg-muted border border-border rounded-lg p-2 text-sm text-foreground outline-none focus:border-indigo-400 resize-none min-h-[100px]"
                                                        autoFocus
                                                    />
                                                    <div className="flex justify-end gap-2">
                                                        <button 
                                                            onClick={cancelEdit}
                                                            className="px-2 py-1 text-xs font-medium text-muted-foreground hover:text-accent-foreground hover:bg-accent rounded"
                                                        >
                                                            Cancel
                                                        </button>
                                                        <button 
                                                ...
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        <h4 className="font-bold text-foreground text-sm">{entry.metadata.title}</h4>
                                                ...
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        <h4 className="font-bold text-foreground text-sm">{entry.metadata.title || "Structured Question"}</h4>
                                                 ...
                                                 <div className="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        <h4 className="font-bold text-foreground text-sm">Wiki Update</h4>
                                                ...
                                                <p className={cn(
                                                    "text-foreground leading-relaxed line-clamp-3",
                                            ...
                                            <div className="flex flex-wrap items-center justify-between mt-2">
                                                <div className="flex items-center gap-2 min-w-0 flex-1 mr-2">
                                                    ...
                                                    <div className="flex items-center gap-2 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                                        {entry.tags && entry.tags.map(tag => (
                                                            <span key={tag} className="text-[10px] text-muted-foreground bg-secondary px-1.5 py-0.5 rounded font-medium whitespace-nowrap shrink-0">
                                                                #{tag}
                                                            </span>
                                                        ))}
                                                    </div>
                                                ...
                                                {isExpanded && (
                                                    <button 
                                                        ...
                                                        className="text-muted-foreground hover:text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-accent rounded-full"
                                                        title="Actions"
                                                    >
