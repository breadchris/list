...
import { X, Plus, GripVertical, Layout, Search, Book, Database, FileText, Settings, CreditCard, Component, MessageSquare, Clock, Layers, ChevronRight, ChevronLeft, Workflow, MoreHorizontal, Edit2, CornerUpRight, ArrowRight, Link, CheckSquare, Trash2, PanelLeftClose, PanelLeftOpen, Globe, BookOpen, Loader2 } from 'lucide-react';
        ...
        <button
            ...
            className={`w-full text-left px-2 py-1.5 rounded-md flex items-center gap-2 transition-colors relative group/pane cursor-pointer
                ${pane.isOpen !== false
                    ...
                    : 'text-muted-foreground hover:text-accent-foreground hover:bg-accent italic'}
            `}
        >
            <GripVertical className="w-3 h-3 text-muted-foreground opacity-0 group-hover/pane:opacity-100 cursor-grab active:cursor-grabbing" />
            ...
            <span className={`text-xs truncate flex-1 ${pane.isOpen === false ? 'line-through decoration-muted-foreground' : ''}`}>
                {pane.title || getPaneDefaultTitle(pane)}
            </span>

            {pane.isOpen === false && (
                <span className="text-[9px] uppercase font-bold text-muted-foreground mr-1">Closed</span>
            )}
    ...
    <div ref={ref} style={{ opacity: isDragging ? 0.5 : 1 }} data-handler-id={handlerId} className="mb-1">
         <button
            ...
            className={`w-full text-left px-3 py-2 flex items-center gap-3 transition-colors relative group
                ${activeRowId === row.id ? 'bg-indigo-50 text-indigo-700' : 'hover:bg-accent text-muted-foreground'}
            `}
         >
            {activeRowId === row.id && (
            ...
            <div className={`shrink-0 flex items-center justify-center w-6 h-6 rounded border text-xs font-medium cursor-grab active:cursor-grabbing
                 ${activeRowId === row.id ? 'border-indigo-200 bg-indigo-100' : 'border-border bg-muted'}
            `}>
                {index + 1}
            </div>
                    ...
                    <div className="min-w-0 flex-1">
                        {editingRowId === row.id ? (
                            <input 
                                ...
                                className="w-full bg-background border border-indigo-300 rounded px-1 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
                            />
                        ) : (
                            <>
                                <div className="text-sm font-medium truncate">{row.title}</div>
                                <div className="text-[10px] text-muted-foreground">{format(new Date(row.createdAt), 'h:mm a')}</div>
                            </>
                        )}
                    </div>
                        ...
                        <div 
                            ...
                            className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full transition-all duration-200 flex items-center justify-center
                                ${confirmDeleteRowId === row.id
                                    ...
                                    : 'opacity-0 group-hover:opacity-100 hover:bg-secondary text-muted-foreground hover:text-red-500'
                                }
                            `}
                        >
                            {confirmDeleteRowId === row.id ? (
         ...
         {isRowSidebarOpen && (activeRowId === row.id || filteredPanes.length > 0) && (
            <div className="ml-9 mr-2 space-y-0.5 mt-1 border-l border-border pl-2">
...
export function Sandbox({
  ...
  useEffect(() => {
      ...
      async function load() {
          try {
              const response = await fetch(`/api/context/state?type=sandbox&groupId=${groupId}`);
  ...
  useEffect(() => {
      ...
      const timeout = setTimeout(async () => {
          try {
              ...
              await fetch(`/api/context/state?type=sandbox&groupId=${groupId}&userId=${userId}`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ value: { rows: cleanRows, activeRowId } })
              });
          } catch (e) {
  ...
  const executeAction = (action: string, target?: string) => {
      ...
      if (action === 'edit_entry') {
          ...
          } else if (target === 'new_timeline') {
             const newSessionId = crypto.randomUUID();
             addPaneToActiveRow('timeline', undefined, newSessionId);
             const text = actionMenu.item.content || actionMenu.item.text;
             if (text) {
                 onAddTimelineEntry(text, newSessionId);
             }
          } else if (target === 'new_graph') {
             addPaneToActiveRow('graph');
             const text = actionMenu.item.content || actionMenu.item.text;
             if (text) {
                 onAddToGraph(text, activeRowId);
             }
          }
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-secondary flex overflow-hidden font-sans">
        ...
        <Command.Dialog
            ...
            label="Go to Pane"
            className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] max-w-[90vw] bg-background rounded-xl shadow-2xl border border-border overflow-hidden z-[100]"
        >
             ...
             <Command.List className="max-h-[300px] overflow-y-auto p-2 scrollbar-hide">
                ...
                <Command.Group heading="Create New" className="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                    <Command.Item onSelect={() => { addPaneToActiveRow('chat'); setIsCmdMenuOpen(false); }} className="flex items-center gap-2 px-2 py-2 text-sm text-foreground rounded-lg cursor-pointer aria-selected:bg-indigo-50 aria-selected:text-indigo-700 transition-colors">
                        <MessageSquare className="w-4 h-4 text-muted-foreground" />
                        <span className="font-medium">Chat</span>
                    </Command.Item>
                    <Command.Item onSelect={() => { addPaneToActiveRow('timeline'); setIsCmdMenuOpen(false); }} className="flex items-center gap-2 px-2 py-2 text-sm text-foreground rounded-lg cursor-pointer aria-selected:bg-indigo-50 aria-selected:text-indigo-700 transition-colors">
                        <Clock className="w-4 h-4 text-muted-foreground" />
                        <span className="font-medium">Timeline</span>
                    </Command.Item>
                    <Command.Item onSelect={() => { addPaneToActiveRow('wiki'); setIsCmdMenuOpen(false); }} className="flex items-center gap-2 px-2 py-2 text-sm text-foreground rounded-lg cursor-pointer aria-selected:bg-indigo-50 aria-selected:text-indigo-700 transition-colors">
                        <FileText className="w-4 h-4 text-muted-foreground" />
                        <span className="font-medium">Wiki</span>
                    </Command.Item>
                    <Command.Item onSelect={() => { addPaneToActiveRow('todo'); setIsCmdMenuOpen(false); }} className="flex items-center gap-2 px-2 py-2 text-sm text-foreground rounded-lg cursor-pointer aria-selected:bg-indigo-50 aria-selected:text-indigo-700 transition-colors">
                        <CheckSquare className="w-4 h-4 text-muted-foreground" />
                        <span className="font-medium">To-Do</span>
                    </Command.Item>
                    <Command.Item onSelect={() => { addPaneToActiveRow('graph'); setIsCmdMenuOpen(false); }} className="flex items-center gap-2 px-2 py-2 text-sm text-foreground rounded-lg cursor-pointer aria-selected:bg-indigo-50 aria-selected:text-indigo-700 transition-colors">
                        <Workflow className="w-4 h-4 text-muted-foreground" />
                        <span className="font-medium">Graph</span>
                    </Command.Item>
                    <Command.Item onSelect={() => { addPaneToActiveRow('iframe'); setIsCmdMenuOpen(false); }} className="flex items-center gap-2 px-2 py-2 text-sm text-foreground rounded-lg cursor-pointer aria-selected:bg-indigo-50 aria-selected:text-indigo-700 transition-colors">
                ...
                {[...rows]
                    ...
                    .map(row => (
                    <Command.Group key={row.id} heading={row.title} className="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                        {row.panes.map(pane => (
                            <Command.Item 
                                key={pane.id}
                                onSelect={() => { 
                                ...
                                className={`flex items-center gap-2 px-2 py-2 text-sm text-foreground rounded-lg cursor-pointer aria-selected:bg-indigo-50 aria-selected:text-indigo-700 transition-colors ${pane.isOpen === false ? 'opacity-50' : ''}`}
                            >
                                {pane.type === 'timeline' && <Layout className="w-4 h-4 text-muted-foreground" />}
                                {pane.type === 'chat' && <MessageSquare className="w-4 h-4 text-muted-foreground" />}
                                {pane.type === 'wiki' && <FileText className="w-4 h-4 text-muted-foreground" />}
                                {pane.type === 'todo' && <CheckSquare className="w-4 h-4 text-muted-foreground" />}
                                {pane.type === 'graph' && <Workflow className="w-4 h-4 text-muted-foreground" />}
                                {pane.type === 'component' && <Component className="w-4 h-4 text-muted-foreground" />}
                                
                                <div className="flex flex-col">
                                    <span className="font-medium">{pane.title || getPaneDefaultTitle(pane)}</span>
                                    {pane.title !== getPaneDefaultTitle(pane) && (
                                        <span className="text-[10px] text-muted-foreground">{getPaneDefaultTitle(pane)}</span>
                                    )}
                                </div>
        ...
        <div 
            className={`hidden md:flex flex-col bg-background border-r border-border transition-all duration-300 z-40 ${isRowSidebarOpen ? 'w-64' : 'w-12'}`}
        >
            <div className="p-3 border-b border-border flex items-center justify-between h-14">
                {isRowSidebarOpen && <span className="text-sm font-semibold text-foreground ml-1">Contexts</span>}
                <button 
                    onClick={() => setIsRowSidebarOpen(!isRowSidebarOpen)}
                    className={`p-1.5 rounded-md transition-all duration-200 ${isRowSidebarOpen ? 'hover:bg-accent text-muted-foreground hover:text-accent-foreground' : 'w-full flex justify-center hover:bg-accent text-muted-foreground hover:text-indigo-600'}`}
            ...
            <div className="flex-1 overflow-y-auto py-2">
                    ...
                    <div className="px-3 mb-2">
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full pl-8 pr-3 py-2 bg-muted border border-border rounded-lg text-xs outline-none focus:border-indigo-400 transition-colors"
                            />
                        </div>
                    </div>
                )}
                ...
                <button
                    ...
                    className={`w-full text-left px-3 py-2 flex items-center gap-3 transition-colors group mt-2 mb-4
                        hover:bg-accent text-muted-foreground hover:text-indigo-600
                    `}
                    title="Create New Context"
                >
                     <div className="shrink-0 flex items-center justify-center w-6 h-6 rounded border border-dashed border-border text-muted-foreground group-hover:border-indigo-300 group-hover:text-indigo-600 transition-colors bg-muted">
        ...
        {isMobileNavOpen && (
            <div className="fixed inset-0 z-50 bg-black/50 md:hidden" onClick={() => setIsMobileNavOpen(false)}>
                <div className="w-64 bg-background h-full shadow-xl p-4 flex flex-col" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2">
                        {rows.map((row, index) => (
                            <button
                                key={row.id}
                                onClick={() => setActiveRowId(row.id)}
                                className={`w-full p-3 rounded-lg text-left flex items-center justify-between ${activeRowId === row.id ? 'bg-indigo-50 text-indigo-700 border border-indigo-100' : 'bg-muted text-foreground border border-border'}`}
                            >
                                <span>{row.title}</span>
                        ...
                        <button 
                            ...
                            className="w-full p-3 rounded-lg border-2 border-dashed border-border text-muted-foreground flex items-center justify-center gap-2 hover:bg-accent hover:text-accent-foreground hover:border-border transition-all"
                        >
        ...
        >
             <div className="absolute inset-0 overflow-hidden bg-secondary">
                     ...
                     >
                         {row.panes.filter(p => p.isOpen !== false).flatMap((pane, i, arr) => {
                             ...
                             <Resizable
                                 ...
                                 handleComponent={{
                                     right: <div 
                                         ...
                                         className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-2 h-12 bg-background shadow-md border border-border rounded-full flex items-center justify-center cursor-col-resize z-20 hover:scale-110 transition-transform"
                                     >
                                         <GripVertical className="w-3 h-3 text-muted-foreground" />
                                     </div>
                                 }}
                                 className="flex flex-col bg-background rounded-xl shadow-xl border border-border/60 overflow-hidden relative group h-full"
                             >
                                 {/* Pane Header */}
                                 <div 
                                    ref={dragHandle}
                                    className="h-10 bg-muted border-b border-border flex items-center justify-between px-3 shrink-0 handle cursor-grab active:cursor-grabbing"
                                    onDoubleClick={(e) => startRenamingPane(e, pane)}
                                 >
                                     <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider min-w-0 flex-1">
                                         ...
                                         {editingPaneId === pane.id ? (
                                             <input 
                                                ...
                                                onClick={(e) => e.stopPropagation()}
                                                className="w-full bg-background border border-indigo-300 rounded px-1 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 normal-case tracking-normal font-normal text-foreground"
                                             />
                                         ) : (
                                         ...
                                         {pane.type === 'chat' && (
                                             <div className="relative ml-2 flex items-center gap-1">
                                                 <button 
                                                    ...
                                                    className="p-1 rounded hover:bg-secondary text-muted-foreground hover:text-accent-foreground transition-colors"
                                                    title="Chat Settings"
                                                 >
                                                 ...
                                                 <button 
                                                    ...
                                                    className={`p-1 rounded hover:bg-secondary transition-colors ${pane.connectedPaneId ? 'text-indigo-600 bg-indigo-50' : 'text-muted-foreground'}`}
                                     ...
                                     <button onClick={() => removePane(row.id, pane.id)} className="text-muted-foreground hover:text-red-500 ml-2">
                                         <X className="w-3.5 h-3.5" />
                                     </button>
                                 </div>
                                 ...
                                 <div className="flex-1 overflow-hidden relative">
                                      ...
                                      {pane.type === 'iframe' && (
                                          <div className="flex flex-col h-full bg-muted">
                                              <div className="p-2 border-b border-border bg-background flex gap-2 shrink-0 items-center">
                                                  <input 
                                                      className="flex-1 px-2 py-1.5 text-xs border border-border rounded text-foreground focus:outline-none focus:border-indigo-400"
                                                  ...
                                                  {pane.url && (
                                                      <button
                                                         ...
                                                         className={`p-1.5 rounded transition-colors ${pane.readerMode ? 'bg-indigo-100 text-indigo-600' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground'}`}
                                              ...
                                              <div className="flex-1 relative bg-secondary overflow-hidden">
                                                  {pane.readerMode ? (
                                                      <div
                                                          className="w-full h-full bg-background overflow-y-auto p-8"
                                                          onContextMenu={(e) => {
                                                              e.preventDefault();
                                      ...
                                      {pane.type === 'component' && pane.componentId && ComponentRegistry[pane.componentId] && (
                                          <React.Suspense fallback={<div className="p-4 text-muted-foreground">Loading...</div>}>
                                              <div className="p-8 flex items-center justify-center min-h-full bg-muted/50">
                             ...
                             !isLast && !isMobile && (
                                 <div key={`divider-${pane.id}`} className="w-4 shrink-0 relative group/divider flex items-center justify-center z-30 self-stretch">
                                     ...
                                     >
                                         {/* Hover Hint Line */}
                                         <div className="absolute inset-x-[7px] inset-y-4 bg-border/50 rounded-full opacity-0 group-hover/divider:opacity-100 transition-opacity duration-200" />
                                         
                                         {/* Plus Button */}
                                         <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-5 h-5 bg-background shadow-sm border border-border rounded-full flex items-center justify-center text-muted-foreground hover:text-indigo-600 hover:border-indigo-300 opacity-0 group-hover/divider:opacity-100 transition-all transform scale-75 group-hover/divider:scale-100 hover:scale-110">
                         ...
                         <div className={`h-full w-20 shrink-0 flex items-center justify-center relative ${isMobile ? '' : 'ml-4'}`}>
                             <button 
                                 ...
                                 className={`w-12 h-12 rounded-full border shadow-sm flex items-center justify-center transition-all duration-300 z-[30] ${isRadialOpen && activeRowId === row.id ? 'bg-indigo-600 border-indigo-600 text-white rotate-45' : 'bg-background border-border text-muted-foreground hover:text-indigo-600 hover:border-indigo-200'}`}
                             >
                                ...
                                {radialItems.map((item, i) => {
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            className={`absolute flex items-center gap-3 bg-background p-2 pr-4 rounded-full shadow-lg border border-border hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-colors group
                                                ${menuDirection === 'right' ? 'flex-row' : 'flex-row-reverse right-0'}
                                            `}
                                            style={{ top: i * 50 }}
                                        >
                                            <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center group-hover:bg-background text-muted-foreground group-hover:text-indigo-600">
                                                <item.icon className="w-4 h-4" />
                                            </div>
                                            <span className="text-sm font-medium text-foreground group-hover:text-indigo-700">{item.label}</span>
                                        </motion.button>
                                    );
                                })}
                            </div>
                 ...
                 <AnimatePresence>
                     {actionMenu && (
                         <div
                            className="fixed z-[70] bg-background rounded-lg shadow-xl border border-border w-56 overflow-hidden py-1 max-h-[calc(100vh-16px)] overflow-y-auto"
                            style={{
                                top: Math.max(8, Math.min(actionMenu.y, window.innerHeight - 420)),
                                left: Math.max(8, Math.min(actionMenu.x, window.innerWidth - 240))
                            }}
                         >
                            ...
                            <div className="p-1">
                                {actionMenu.item.content && actionMenu.sourcePaneId && paneRefs.current[actionMenu.sourcePaneId]?.startEditing && (
                                    <>
                                        ...
                                        <button onClick={() => {
                                            ...
                                            setActionMenu(null);
                                        }} className="w-full text-left px-2 py-1.5 hover:bg-red-50 hover:text-red-600 rounded flex items-center gap-2 text-sm text-foreground">
                                ...
                                <button onClick={() => executeAction('new_row')} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                     <Layout className="w-4 h-4" />
                                     <span>New Context with this...</span>
                                </button>
                                <button onClick={() => executeAction('send_to_row', activeRowId)} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                ...
                                <button onClick={() => executeAction('open_in_pane', 'new_chat')} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                     <MessageSquare className="w-4 h-4" />
                                     <span>Discuss in New Chat</span>
                                </button>
                                <button onClick={() => executeAction('open_in_pane', 'new_wiki')} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                     <FileText className="w-4 h-4" />
                                     <span>Create Wiki Page</span>
                                </button>
                                <button onClick={() => executeAction('open_in_pane', 'new_todo')} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                     <CheckSquare className="w-4 h-4" />
                                     <span>Add to New Todo List</span>
                                </button>
                                <button onClick={() => executeAction('open_in_pane', 'new_iframe')} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                     <Globe className="w-4 h-4" />
                                     <span>Open in New Iframe</span>
                                </button>
                                <button onClick={() => executeAction('open_in_pane', 'new_timeline')} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                     <Clock className="w-4 h-4" />
                                     <span>Add to New Timeline</span>
                                </button>
                                <button onClick={() => executeAction('open_in_pane', 'new_graph')} className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground">
                                     <Workflow className="w-4 h-4" />
                                     <span>Add to New Graph</span>
                                </button>
                 ...
                 <AnimatePresence>
                     {connectMenu && (
                         <div 
                            className="fixed z-[70] bg-background rounded-lg shadow-xl border border-border w-64 overflow-hidden py-1"
                            style={{ top: connectMenu.y, left: Math.min(connectMenu.x, window.innerWidth - 270) }}
                         >
                            <div className="px-3 py-2 border-b border-border bg-muted">
                                <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Connect to...</div>
                            </div>
                            <div className="p-1 max-h-[200px] overflow-y-auto">
                                <button 
                                    ...
                                    className="w-full text-left px-2 py-1.5 hover:bg-red-50 hover:text-red-600 rounded flex items-center gap-2 text-sm text-muted-foreground italic mb-1"
                                >
                                     <X className="w-4 h-4" />
                                ...
                                {rows.find(r => r.id === connectMenu.rowId)?.panes
                                    ...
                                    <button 
                                        ...
                                        className="w-full text-left px-2 py-1.5 hover:bg-indigo-50 hover:text-indigo-600 rounded flex items-center gap-2 text-sm text-foreground"
                                    >
                                        {p.type === 'wiki' && <FileText className="w-4 h-4 text-muted-foreground" />}
                                        {p.type === 'graph' && <Workflow className="w-4 h-4 text-muted-foreground" />}
