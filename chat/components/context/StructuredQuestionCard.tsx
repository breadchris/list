...
export function StructuredQuestionCard({ data, onSelect, onRefine }: StructuredQuestionCardProps) {
  ...
  return (
    <div className="w-full max-w-2xl bg-background rounded-xl overflow-hidden shadow-sm border border-border font-sans my-2">
       {/* Header */}
            ...
            return (
                <div key={q.id || qIndex} className="space-y-3">
                    <h3 className="text-base font-semibold text-foreground leading-snug">
                        {questions.length > 1 && <span className="text-muted-foreground mr-2">{qIndex + 1}.</span>}
                        {q.text}
                    </h3>
                                ...
                                <button
                                    ...
                                    className={clsx(
                                        ...
                                        isSelected 
                                            ...
                                            : "bg-background border-border hover:bg-accent hover:border-border"
                                    )}
                                >
                                    <div className={clsx("mt-0.5 shrink-0", isSelected ? "text-indigo-600" : "text-muted-foreground")}>
                                        {isSelected ? <CheckCircle className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
                                    </div>
                                    <div>
                                        <div className={clsx("font-medium text-sm", isSelected ? "text-indigo-900" : "text-foreground")}>
                                            {option.label}
                                        </div>
                                        {option.description && (
                    ...
                    {(q.allowCustomInput !== false) && (
                        <div className="mt-2">
                            <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1.5 block ml-1">
                                Or custom response
                            </label>
                            <div className="relative">
                                <textarea
                                    ...
                                    className={clsx(
                                        "w-full rounded-lg p-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all min-h-[80px] resize-y border",
                                        currentAnswer?.isCustom ? "bg-background border-indigo-200" : "bg-muted border-border"
                                    )}
                                />
