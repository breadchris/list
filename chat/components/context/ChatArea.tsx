    ...
    <div className="flex flex-col h-full bg-background relative">
      ...
      <AnimatePresence>
        {showSettings && session && (
            <motion.div
                ...
                className="absolute top-16 right-4 w-72 bg-background rounded-xl shadow-xl border border-border z-30 overflow-hidden"
            >
                <div className="flex items-center justify-between px-4 py-3 bg-muted border-b border-border">
                    ...
                    <button onClick={() => setShowSettings(false)} className="text-muted-foreground hover:text-accent-foreground p-1 hover:bg-accent rounded-full transition-colors">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                        ...
                        {session.constraints && Object.entries(session.constraints).length > 0 ? (
                            <div className="space-y-3">
                                {Object.entries(session.constraints).map(([key, value]) => (
                                    <div key={key} className="bg-muted border border-border rounded-lg p-3 group hover:border-indigo-100 transition-colors">
                                        <div className="flex items-center gap-2 mb-1.5">
                                            <div className="w-1.5 h-1.5 rounded-full bg-indigo-400" />
                                            <span className="text-xs font-bold text-muted-foreground uppercase tracking-wide">{key}</span>
                                        </div>
                                        <div className="text-sm text-foreground leading-relaxed break-words pl-3.5 border-l-2 border-border group-hover:border-indigo-200 transition-colors">
                                            {value}
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="text-sm text-muted-foreground italic bg-muted border border-border border-dashed rounded-lg p-4 text-center">
      ...
      <div 
            ...
            return (
                ...
                >
                    <div 
                        ...
                        ${isUser ? 'bg-slate-800 text-white' : 'bg-background border border-border text-muted-foreground'}`}
                    ...
                    <div className={`flex flex-col max-w-[85%] md:max-w-[75%] ${isUser ? 'items-end' : 'items-start'}`}>
                        ...
                        <div className="relative group/bubble">
                                    ...
                                    <ReactMarkdown
                                        components={{
                                            ...
                                            code: ({node, inline, className, children, ...props}: any) => {
                                                if (inline) {
                                                    return <code className="bg-secondary rounded px-1 py-0.5 text-xs font-mono text-foreground" {...props}>{children}</code>;
                                                }
                            ...
                            <button 
                                ...
                                className={`absolute top-2 ${isUser ? '-left-10' : '-right-10'} p-2 text-muted-foreground hover:text-indigo-600 bg-background rounded-full shadow-sm border border-border opacity-0 group-hover:opacity-100 transition-all scale-90 hover:scale-100 z-10`}
                                title="Actions"
                            >
        ...
        {isTyping && (
          ...
          >
            <div 
                className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 bg-background border border-border overflow-hidden"
            ...
            <div className="bg-muted border border-border px-4 py-3 rounded-2xl rounded-tl-none">
      ...
      <div className="p-4 bg-background border-t border-border">
        <div className="max-w-4xl mx-auto w-full">
                    ...
                    <Tooltip.Root key={p.id}>
                      ...
                      <Tooltip.Portal>
                        ...
                        >
                          ...
                          <div className="text-muted-foreground max-w-[200px] leading-tight">{p.description}</div>
                ...
                <Tooltip.Root>
                  <Tooltip.Trigger asChild>
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all whitespace-nowrap border border-dashed border-border text-muted-foreground hover:text-foreground hover:border-border hover:bg-accent shrink-0 ml-1"
                    >
                  ...
                  <Tooltip.Portal>
                    ...
                    >
                      ...
                      <div className="text-muted-foreground max-w-[200px] leading-tight">Create, edit, or remove custom AI personalities</div>
                      <Tooltip.Arrow className="fill-popover" />
                    </Tooltip.Content>
                  </Tooltip.Portal>
                </Tooltip.Root>
            ...
            <form 
                ...
                className="relative flex items-center gap-2 bg-muted p-2 rounded-xl border border-border focus-within:ring-2 focus-within:ring-offset-1 focus-within:border-transparent transition-all"
                style={{ 
            ...
            <input
                ...
                className="flex-1 bg-transparent border-none focus:ring-0 px-3 py-2 text-foreground placeholder:text-muted-foreground outline-none"
