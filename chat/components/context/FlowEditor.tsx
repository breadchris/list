...
import { Save, MousePointer2, StickyNote, Square, Type, Hand, FileText } from 'lucide-react';
import { clsx } from 'clsx';
import dagre from 'dagre';
import { StickyNode, ShapeNode, TextNode, WikiNode } from './FlowNodes';
import SimpleFloatingEdge from './FlowEdges';

type Tool = 'select' | 'hand' | 'sticky' | 'shape' | 'text' | 'wiki';
...
function FlowToolbar({ 
    ...
    return (
        <Panel position="bottom-center" className="mb-8 flex flex-col items-center gap-4">
             {/* Status Pill */}
             <div className="bg-background/80 backdrop-blur-sm px-3 py-1 rounded-full shadow-sm border border-border text-[10px] text-muted-foreground flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 bg-background p-2 rounded-2xl shadow-2xl border border-border ring-1 ring-black/5">
                <button 
                    onClick={() => setTool('select')} 
                    className={clsx("p-3 rounded-xl transition-all", currentTool === 'select' ? "bg-indigo-600 text-white shadow-md scale-105" : "hover:bg-accent text-muted-foreground")} 
                    title="Select (V)"
                >
                ...
                <button 
                    ...
                    className={clsx("p-3 rounded-xl transition-all", currentTool === 'hand' ? "bg-indigo-600 text-white shadow-md scale-105" : "hover:bg-accent text-muted-foreground")} 
                    title="Hand (H)"
                >
                ...
                <button 
                    ...
                    className={clsx("p-3 rounded-xl transition-all", currentTool === 'sticky' ? "bg-indigo-600 text-white shadow-md scale-105" : "hover:bg-accent text-muted-foreground")} 
                    title="Sticky Note (S)"
                >
                ...
                <button 
                    ...
                    className={clsx("p-3 rounded-xl transition-all", currentTool === 'shape' ? "bg-indigo-600 text-white shadow-md scale-105" : "hover:bg-accent text-muted-foreground")} 
                    title="Shape (R)"
                >
                ...
                <button
                    ...
                    className={clsx("p-3 rounded-xl transition-all", currentTool === 'text' ? "bg-indigo-600 text-white shadow-md scale-105" : "hover:bg-accent text-muted-foreground")}
                    title="Text (T)"
                >
                    <Type className="w-5 h-5" />
                </button>
                <button
                    onClick={() => setTool('wiki')}
                    className={clsx("p-3 rounded-xl transition-all", currentTool === 'wiki' ? "bg-indigo-600 text-white shadow-md scale-105" : "hover:bg-accent text-muted-foreground")}
                    title="Wiki Editor (W)"
                >
                ...
                <button
                    ...
                    className={clsx(
                        ...
                        "text-muted-foreground hover:bg-accent hover:text-indigo-600"
                    )}
                    title="Save Version"
                >
...
const FlowCanvas = forwardRef<IPane, {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            switch(e.key.toLowerCase()) {
                ...
                case 'w': setCurrentTool('wiki'); break;
            }
        };
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                const response = await fetch(`/api/context/state?type=flow&groupId=${groupId}&sessionId=${sessionId}`);
        ...
        if (pendingActions && pendingActions.length > 0 && isLoaded) {
            ...
            pendingActions.forEach(action => {
                if (action.type === 'add_flow_node') {
                    ...
                    const source = newNodes.find(n => (n.data as any).label?.toLowerCase().includes(action.sourceLabel.toLowerCase()));
                    const target = newNodes.find(n => (n.data as any).label?.toLowerCase().includes(action.targetLabel.toLowerCase()));
                    if (source && target) {
                        newEdges.push({
                            id: crypto.randomUUID(),
                            source: source.id,
    ...
    const saveToDb = async (currentNodes: Node[], currentEdges: Edge[]) => {
        try {
            await fetch(`/api/context/state?type=flow&groupId=${groupId}&userId=${userId}`, {
                method: 'POST',
    ...
    const handleManualSave = async () => {
        ...
        try {
            ...
            await fetch(`/api/context/state?type=flow-version&groupId=${groupId}&userId=${userId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const onPaneClick = useCallback((event: React.MouseEvent) => {
        if (currentTool === 'sticky' || currentTool === 'shape' || currentTool === 'text' || currentTool === 'wiki') {
            ...
            } else if (currentTool === 'wiki') {
                newNode = {
                    id, type: 'wiki',
                    position: { x: position.x - 200, y: position.y - 125 },
                    data: { groupId, userId, supabase },
            ...
            if (currentTool !== 'text' && currentTool !== 'wiki') {
                newNode.position = { x: newNode.position.x - 75, y: newNode.position.y - 75 };
            }

            setNodes((nds) => [...nds, newNode]);
        }
    }, [currentTool, screenToFlowPosition, setNodes, groupId, userId, supabase]);

    return (
        <div className="h-full w-full relative bg-muted cursor-default" style={{ width: '100%', height: '100%' }}>
            <ReactFlow
                defaultNodes={[]}
                defaultEdges={[]}
            ...
            >
                ...
                <Controls showInteractive={false} className="!bg-background !border-border !shadow-lg !rounded-lg" />
...
export const FlowEditor = forwardRef<IPane, {
    sessionId: string;
    groupId: string;
    userId: string;
    supabase: SupabaseClient;
