...
export function PersonaManager({ personas, onSave, onClose, projectId, publicAnonKey }: PersonaManagerProps) {
  ...
  const startEditing = (persona: Persona) => {
    ...
    fetch(`https://${projectId}.supabase.co/functions/v1/make-server-61781242/editor/state`, {
        method: 'POST',
        headers: {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          const res = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-61781242/upload`, {
              method: 'POST',
  ...
  const toggleTestMode = async () => {
      if (!isTestMode) {
          ...
          const res = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-61781242/editor/state?sessionId=${editorSessionId}`, {
             headers: { 'Authorization': `Bearer ${publicAnonKey}` }
          });
          const data = await res.json();
          let draftPrompt = "";
  ...
  const handleTestMessage = async (text: string) => {
      ...
      try {
          ...
          const promptRes = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-61781242/editor/state?sessionId=${editorSessionId}`, {
             headers: { 'Authorization': `Bearer ${publicAnonKey}` }
          });
  ...
  return (
    <div className="fixed inset-0 bg-slate-100 z-[100] flex flex-col overflow-hidden animate-in fade-in duration-200">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-xl font-semibold text-slate-800">Persona Manager</h2>
                <p className="text-sm text-slate-500">Design custom AI assistants</p>
            </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {/* Sidebar List */}
          <div className={cn(
              "w-80 bg-white border-r border-slate-200 flex flex-col overflow-hidden transition-all duration-300",
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                  {localPersonas.map((p, idx) => (
                      ...
                      >
                           <div className="shrink-0 cursor-grab text-slate-300 hover:text-slate-500">
              ...
              {editingId && (
                  <>
                    ...
                    <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shrink-0">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="relative group/avatar cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                                ...
                                />
                                <div className={cn(
                                    "w-12 h-12 rounded-full flex items-center justify-center text-white shrink-0 shadow-sm overflow-hidden bg-slate-200 border-2 border-transparent hover:border-indigo-300 transition-all",
                            ...
                            <input 
                                ...
                                className="text-lg font-bold text-slate-800 bg-transparent border-none focus:ring-0 placeholder:text-slate-300 w-48"
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <input 
                                    ...
                                    className="text-sm text-slate-700 bg-slate-50 border-none rounded-md px-2 py-1 focus:ring-1 focus:ring-indigo-500 w-32"
                    ...
                    <div className="flex-1 overflow-hidden relative">
                        {isTestMode ? (
                            <div className="h-full bg-white flex flex-col max-w-3xl mx-auto border-x border-slate-100 shadow-sm">
                                    ...
                                    <div className="p-4 flex flex-col gap-4 flex-1 overflow-hidden">
                                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden">
                                            <div className="px-4 py-2 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                                                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">System Prompt Configuration</span>
                                            </div>
                                        ...
                                        {/* Meta Fields */}
                                        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm shrink-0 grid grid-cols-2 gap-4">
                                            <div>
                                            ...
                                            <div>
                                                <label className="block text-xs font-medium text-slate-500 mb-1">Initial Greeting</label>
                                                <input 
