...
export const StickyNode = memo(({ id, data, selected }: any) => {
    ...
    return (
        <>
            <NodeToolbar isVisible={selected} position={Position.Top} className="flex gap-1 bg-white p-1.5 rounded-lg shadow-xl border border-slate-200">
                {STICKY_COLORS.map(c => (
                    <button 
                        key={c} 
                ...
                <button onClick={() => deleteElements({ nodes: [{ id }] })} className="text-slate-400 hover:text-red-500 p-0.5">
            ...
            >
                ...
                <textarea 
                    className="w-full h-full bg-transparent resize-none outline-none text-center font-medium text-slate-800 placeholder-slate-800/30 text-lg leading-relaxed nodrag cursor-text" 
                    placeholder="Idea..."
                    value={data.label}
...
export const ShapeNode = memo(({ id, data, selected }: any) => {
    ...
    return (
        <>
            <NodeResizer minWidth={100} minHeight={50} isVisible={selected} lineClassName="border-indigo-400" handleClassName="h-2.5 w-2.5 bg-white border-2 border-indigo-500 rounded-sm" />
            
            <NodeToolbar isVisible={selected} position={Position.Top} className="flex gap-1 bg-white p-1.5 rounded-lg shadow-xl border border-slate-200 items-center">
                 <div className="flex gap-1">
                    <button onClick={() => updateShape('rectangle')} className={`p-1 rounded hover:bg-slate-100 ${shape === 'rectangle' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500'}`} title="Rectangle"><div className="w-4 h-4 border-2 border-current" /></button>
                    <button onClick={() => updateShape('rounded')} className={`p-1 rounded hover:bg-slate-100 ${shape === 'rounded' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500'}`} title="Rounded"><div className="w-4 h-4 border-2 border-current rounded-md" /></button>
                    <button onClick={() => updateShape('circle')} className={`p-1 rounded hover:bg-slate-100 ${shape === 'circle' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500'}`} title="Circle"><div className="w-4 h-4 border-2 border-current rounded-full" /></button>
                 </div>
                 <div className="w-px h-5 bg-slate-200 mx-1" />
                 <div className="flex gap-1">
                    {SHAPE_COLORS.map(c => (
            ...
            <div className={`w-full h-full min-w-[50px] min-h-[50px] relative group`}>
                <div 
                    className={`absolute inset-0 border-2 transition-all shadow-sm ${selected ? 'border-indigo-500' : 'border-slate-800'}`}
                    style={getShapeStyles()}
                />
                
                <div className="absolute inset-0 flex items-center justify-center p-2 z-10">
                    <textarea 
                        className="w-full bg-transparent resize-none outline-none text-center font-bold text-slate-800 placeholder-slate-400/50 nodrag cursor-text overflow-hidden" 
                        placeholder="Label"
                        value={data.label}
                        onChange={(evt) => updateLabel(evt.target.value)}
                        style={{ height: '100%' }}
    ...
    return (
        <>
            ...
            <div className={`min-w-[50px] relative group`}>
                ...
                <textarea 
                    className="w-full bg-transparent resize-none outline-none text-center font-medium text-slate-800 text-2xl placeholder-slate-300 nodrag cursor-text overflow-hidden" 
                    placeholder="Type..."
                    value={data.label}
                    onChange={(evt) => {
