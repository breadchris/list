...
export function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden my-4 max-w-2xl mx-auto">
      {/* Header */}
      <div className="bg-orange-50 p-6 border-b border-orange-100">
        <div className="flex items-start justify-between mb-4">
          <div>
             <h2 className="text-2xl font-bold text-slate-800 font-serif mb-2">{recipe.title}</h2>
             <p className="text-slate-600 text-sm italic">{recipe.description}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          <div className="flex items-center gap-1.5 text-slate-700 bg-white/60 px-2 py-1 rounded-md">
      ...
      <div className="p-6 grid md:grid-cols-[1fr,1.5fr] gap-8">
        {/* Ingredients */}
        <div>
           <h3 className="font-bold text-slate-800 mb-4 border-b pb-2">Ingredients</h3>
        ...
        {/* Instructions */}
        <div>
           <h3 className="font-bold text-slate-800 mb-4 border-b pb-2">Instructions</h3>
                 ...
                 >
                    <div className="shrink-0 mt-0.5">
                       {isCompleted ? (
                           ...
                           <div className="w-5 h-5 rounded-full border-2 border-slate-300 text-slate-400 flex items-center justify-center text-xs group-hover:border-orange-400 group-hover:text-orange-500 transition-colors">
                             {inst.step}
                           </div>
                       )}
                    </div>
                    <p className={`text-sm leading-relaxed ${isCompleted ? 'text-slate-500 line-through decoration-slate-300' : 'text-slate-700'}`}>
                        {inst.text}
                    </p>
                 </div>
               );
