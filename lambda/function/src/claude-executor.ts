...
/**
 * Execute Claude Code asynchronously with job tracking
 * This function runs in the background and updates job status in S3
 */
export async function executeClaudeCodeAsync(
	job_id: string,
	options: ClaudeExecutionOptions,
	sessionManager: any
): Promise<void> {
	try {
		...
		process.stderr.write(`[Job ${job_id}] Starting execution\n`);
		await sessionManager.storeJobStatus(job_id, 'running');

		// Execute Claude Code
		const result = await executeClaudeCode(options);

		// Store the result in S3
		process.stderr.write(`[Job ${job_id}] Execution completed, storing result\n`);
		await sessionManager.storeJobResult(job_id, result);

		// Update status to completed
		await sessionManager.storeJobStatus(job_id, 'completed');
		process.stderr.write(`[Job ${job_id}] Job completed successfully\n`);

	} catch (error) {
		// Store error result
		const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
		process.stderr.write(`[Job ${job_id}] Job failed: ${errorMessage}\n`);
		...
		await sessionManager.storeJobResult(job_id, errorResult);
		await sessionManager.storeJobStatus(job_id, 'error', errorMessage);
	}
}
