...
interface LexicalContentInputProps {
  ...
  chatMode?: boolean;
  chatValue?: string;
  onChatChange?: (e: React.ChangeEvent<HTMLInputElement>) => void;
}
...
export const LexicalContentInput = forwardRef<LexicalContentInputRef, LexicalContentInputProps>(({
  ...
  // If in chat mode, render a simple controlled input
  if (chatMode) {
    return (
      <div className="relative w-full">
        <input
          ...
          disabled={disabled}
          className="w-full px-4 py-2 bg-gray-700 text-white placeholder-gray-400 rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-teal-500 disabled:opacity-50"
          onKeyDown={(e) => {
            if (e.key === 'Enter' && !e.shiftKey) {
              e.preventDefault();
              if (chatValue?.trim()) {
                onSubmit(chatValue.trim(), []);
              }
            }
          }}
        />
